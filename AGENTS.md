...
이 프로젝트는 텔레그램 브릿지(`~/.local/share/tg-Codex-bridge/bot.py`)를 통해 작업 지시를 받을 수 있습니다. 브릿지는 `Codex -p` 가 **완전히 끝나야** 결과를 회신하는 구조라, 작업 도중 오래 걸리는 명령을 시작하면 사용자는 진행 사실을 알 수 없습니다(내용 없는 하트비트만 보임).
