...
"""IANA 타임존 → ISO 3166-1 alpha-2 국가 코드.

앱이 보내는 tz(Intl.DateTimeFormat().resolvedOptions().timeZone)로 사용자의
국가를 추정한다. tzdb zone.tab 에서 생성했고, 구형 ICU 가 반환하는 옛 이름
(Asia/Calcutta 등)도 포함한다. UTC/Etc/* 처럼 국가가 없는 존은 None.
"""
...
def country_for_tz(tz: str | None) -> str | None:
    """타임존 문자열로 국가 코드를 추정한다. 모르면 None."""
    if not tz:
        return None
    return TZ_COUNTRY.get(tz.strip())
