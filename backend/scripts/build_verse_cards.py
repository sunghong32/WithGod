...
FRAME_TAIL = re.compile(
    ...
    # "할지니라 하라"(민수기 6:26 아론의 축복)는 성경 전체에서 1절뿐이라 안전하다.
    # 그냥 "하라"로 넓히면 안 된다 — 268절이 걸리고 그 중 로마서 12:2,
    # 에베소서 4:29·4:32, 빌립보서 2:14 처럼 멀쩡한 명령형 카드가 잘린다.
    r"\s+(?:하시니라|하셨느니라|하셨으니|하셨나이다|하시었더라|하시더라|하시니|하시고|하신대|하시매|하니라"
    r"|하였더라|하더라|하였느니라|하셨더라|할지니라 하라)[!?.]*$"
)
